...
function PeriodModal({
    ...
    useEffect(() => {
        if (period) {
            ...
            const branchId = typeof period.branchId === "object" ? period.branchId?._id : period.branchId;
    ...
    const selectedBranchName = branches.find((b) => b._id === selectedBranch)?.name || "Tất cả cơ sở";
...
export default function AdminEvaluationsScreen() {
    ...
    const fetchStats = useCallback(async (periodId: string) => {
        try {
            setLoading(true);
            const response = await api.get(`/evaluation-periods/${periodId}/statistics`);
            setClassStats(response.data.byClass || []);
        } catch (error) {
        ...
        Alert.alert(
            ...
            [
                { text: "Hủy", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            await api.delete(`/evaluation-periods/${period._id}`);
                            fetchPeriods();
    ...
    const handleSavePeriod = async (data: any) => {
        try {
            ...
            if (editingPeriod) {
                await api.patch(`/evaluation-periods/${editingPeriod._id}`, data);
                Alert.alert("Thành công", "Đã cập nhật kỳ đánh giá");
            } else {
                await api.post("/evaluation-periods", data);
                Alert.alert("Thành công", "Đã tạo kỳ đánh giá mới");
            }
            setShowModal(false);
            fetchPeriods();
        } catch (error: any) {
            Alert.alert("Lỗi", error.response?.data?.message || "Không thể lưu kỳ đánh giá");
        } finally {
                                ...
                                <View style={styles.classStatHeader}>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <Text style={styles.classStatInfo}>
                                            {item.totalEvaluations} đánh giá • {item.completionRate}% hoàn thành
                                        </Text>
                                    </View>
                                </View>
                                ...
                                {item.teachers.length > 0 && (
                                    <View style={styles.teachersList}>
                                        {item.teachers.map((teacher, idx) => (
                                            <View key={teacher.teacherId} style={styles.teacherStatRow}>
